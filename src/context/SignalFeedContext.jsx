...
const SignalFeedContext = createContext(null)

export function SignalFeedProvider({ children }) {
  ...
  // Load on mount — persists for entire app session
  useEffect(() => { loadSchedules(); loadResults() }, [loadSchedules, loadResults])

  // SSE feed — single persistent connection for the whole app
  useEffect(() => {
  ...
  return (
    <SignalFeedContext.Provider value={{ schedules, results, running, saveSchedule, deleteSchedule, runNow, loadSchedules }}>
      {children}
    </SignalFeedContext.Provider>
  )
}

export function useSignalFeedContext() {
  const ctx = useContext(SignalFeedContext)
  if (!ctx) throw new Error('useSignalFeedContext must be used inside SignalFeedProvider')
  return ctx
}
